...
COUNT=0
for ECM in ${ECMS[@]}; do
    NUM_UP=$(ls -l /besfs/groups/cal/dedx/$USER/bes/DDbarPi-DT/run/DDbarPi/samples/mc/$TYPE/$ECM | grep "txt" | wc -l)
    JobText_SaveDir=/besfs/groups/cal/dedx/$USER/bes/DDbarPi-DT/run/DDbarPi/jobs_text/mc/$TYPE/$ECM
    ROOT_SaveDir=/besfs/groups/cal/dedx/$USER/bes/DDbarPi-DT/run/DDbarPi/rootfile/mc/$TYPE/$ECM
    ...
    do
        file_list=incMC_${TYPE}_${ECM}_DDbarPi_20G-${num}.txt
        rootfile=DDbarPi_incMC_${TYPE}_$ECM-${num}.root
        jobOptions=jobOptions_DDbarPi_incMC_${TYPE}_$ECM-${num}.txt
